...
import { reqBody, addFuncionario } from "./cadastrarFuncionario.js";
...
const server = http.createServer((request, response) => {
  ...
  fs.readFile("empregados.json", "utf8", (err, data) => {
    ...
    if (url === "/empregados" && method === "GET") {
      ...
      // contar o número total de funcionários cadastrados
      lerDadosFuncionarios((err, funcionario) => {
        if (err) {
          response.writeHead(500, { "Contet-Type": "application/json" });
          response.end(JSON.stringify({ message: "Erro ao ler o arquivo" }));
      ...
      // listar todos os funcionários de um determinado cargo
      ...
      lerDadosFuncionarios((err, funcionario) => {
        if (err) {
          response.writeHead(500, { "Contet-Type": "application/json" });
          response.end(
            JSON.stringify({ message: "Funcionario nao encontrado" })
          );
        }
        const jsonData = JSON.parse(data);
      ...
      url.startsWith("/empregados/porHabilidade/") &&
      method === "GET"
    ) {
      // listar todos os funcionários que possuam uma determinada habilidade
      ...
      lerDadosFuncionarios((err, funcionario) => {
        ...
        const funcionarioPorHabilidade = jsonData.filter((funcionario) =>
          funcionario.habilidades.inclues(habilidade)
        );
        if (funcionarioPorHabilidade.length === 0) {
          response.writeHead(404, { "Content-Type": "application/json" });
          response.end(
            JSON.stringify({
              message: "Não existe funcinário com tal habilidade",
            })
          );
          return;
        }
      ...
      // listar todos os funcionários dentro de uma faixa salarial especificada
      /*Requisições
                boby -> JSON -> POST
      ...
      lerDadosFuncionarios((err, funcionario) => {
        ...
        if (funcionarioPorFaixaSalarial.length === 0) {
          ...
          response.end(
            JSON.stringify({
              message: "Não existe funcioário com essa faixa salarial",
            })
          );
          return;
        }
      ...
      // detalhes de um funcionário específico com base em seu ID
      ...
      lerDadosFuncionarios((err, funcionario) => {
        ...
        const jsonData = JSON.parse(data);
        const indexEmpregado = jsonData.findIndex(
          (empregado) => empregado.id === id
        );
      ...
      return reqBody(req, (novoFuncionario, err) => {
        if (err) {
          response.writeHead(404, { "Content-Type": "application/json" });
          response.end(JSON.stringify({ message: "Dados inválidos " }));
          return;
        }
        return addFuncionario(novoFuncionario);
      });
    } else if (url.startsWith("/empregados/") && method === "PUT") {
      // atualizar as informações de um funcionário específico com base em seu ID
      const id = parseInt(url.split("/")[2]);
      ...
      request.on("end", () => {
        fs.readFile("empregados.json", "utf8", (err, data) => {
          ...
          const jsonData = JSON.parse(data);
          const indexFuncionario = jsonData.findIndex(
          ...
          if (indexFuncionario === -1) {
            response.writeHead(404, { "Content-Type": "application/json" });
            response.end(
              JSON.stringify({ message: "Funcionário não encontrado!" })
            );
          }
      ...
      // excluir um funcionário específico com base em seu ID
      const id = parseInt(url.split("/")[2]);
      fs.readFile("empregados.json", "utf8", (err, data) => {
        if (err) {
          response.writeHead(500, { "Contet-Type": "application/json" });
